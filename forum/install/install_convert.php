...
class install_convert extends module
{
	...
	*/
	function list_convertors($sub)
	{
		...
		$handle = @opendir('../../phpBB3/install/convertors');
		...
		while ($entry = readdir($handle))
		{
			...
			{
				include('../../phpBB3/install/convertors' . $entry);
	...
	*/
	function get_convert_settings($sub)
	{
		...
		if (!file_exists('../../phpBB3/install/convertors/convert_' . $convertor_tag . '.' . $phpEx))
		{
			$this->p_master->error($lang['CONVERT_NOT_EXIST'], __LINE__, __FILE__);
		}

		include('../../phpBB3/install/convertors/convert_' . $convertor_tag . '.' . $phpEx);
...
//					'S_HIDDEN'	=> $s_hidden_fields,
	...
	*/
	function convert_data($sub)
	{
		...
		if (file_exists('../../phpBB3/install/convertors/functions_' . $convert->convertor_tag . '.' . $phpEx))
		{
			include('../../phpBB3/install/convertors/functions_' . $convert->convertor_tag . '.' . $phpEx);
		}

		$get_info = true;
		include('../../phpBB3/install/convertors/convert_' . $convert->convertor_tag . '.' . $phpEx);
