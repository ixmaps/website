...
class IXmapsGeoCorrection
{
	...
	public static function getIpAddrInfo($limit, $type=0, $ip='', $offset=0)
	{
		...
		// select ips not geo corrected, with valid coordinates, but with not city name
			...
			$sql1 = "SELECT ip_addr, asnum, hostname, lat, long, mm_country, mm_region, mm_city, gl_override FROM ip_addr_info WHERE mm_city = '' and p_status<>'F' and p_status<>'G' and p_status<>'U' and lat <> 0 and gl_override is not null order by ip_addr OFFSET $offset LIMIT $limit;";
		}
