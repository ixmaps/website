...
class IXmapsGeoCorrection
{
	...
	public static function getIpAddrInfo($limit)
	{
		...
		$sql1 = "SELECT * FROM ip_addr_info WHERE ip_addr > '".$ip."' ORDER BY ip_addr LIMIT $limit";
	...
	*/
	public function insertLogIpAddrInfo($data)
	{
		global $dbconn;
		...
		foreach ($data as $key => $ip) {
			...
			$ip['dis_mm_first_updated'] = IXmapsGeoCorrection::distanceBetweenCoords(
				$ip['mm_lat'], $ip['mm_long'], $geoIp['geoip']['latitude'], $geoIp['geoip']['longitude']);

			$ip['dis_mm_first_corrected'] = IXmapsGeoCorrection::distanceBetweenCoords(
